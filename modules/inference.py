...
def load_models():
    ...
    model_nf.load_state_dict(torch.load(PROJECT_ROOT / "trained_models/CVAE_optimal_NF.pt", map_location=genargs.device))

    model_nnf = CVAE_nnf(args=genargs).to(genargs.device)
    model_nnf.load_state_dict(torch.load(PROJECT_ROOT / "trained_models/CVAE_optimal_NNF.pt", map_location=genargs.device))

    model_mse = CVAE_nf(args=genargs).to(genargs.device)
    model_mse.load_state_dict(torch.load(PROJECT_ROOT / "trained_models/CVAE_MSE.pt", map_location=genargs.device))
...
if __name__ == '__main__':
    ...
    models = load_models()
    attributes = load_attributes_data(file_path=PROJECT_ROOT / "Inference_data/list_attr_celeba_test.csv")
    print('Models loaded.')
    ...
    output_path = PROJECT_ROOT / "Inference_data" / "generated_samples.png"
    plt.savefig(output_path, dpi=150, bbox_inches='tight')
    print(f"Generated figure saved to: {output_path}")
    plt.close()
